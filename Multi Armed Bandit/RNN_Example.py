...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb  2 01:41:52 2019
...
def update_state(xk, sk, wx, wRec):
    """ 
    Compute state k from the previous state (sk) and current input(xk), by use of he input weights (wx) and recursive weights (wRec)
    """
    return xk * wx + sk *wRec

def forward_states(X,wx, wRec):
    """
    Unfold the network and compute all state activations given the input X, input weights (wx), and recursive weights
    (wRec). Return the state activations in a matrix, the last column S[:,-1] contains the final activations.
    """
    # Initialise the matrix that holds all states for all input sequences. The initial state s0 is set to 0
    S = np.zeros((X.shape[0], X.shape[1]+1))
    # Use the recurrence relation defined by update_state to updatethe states through time
...
def backward_gradient(X, S, grad_out, wRec):
    ...
    Accumulate the parameter gradients for wX and wRec by for each layer by addition. Return the parameter
    gradients a a tuple, and the gradients at the output of each layer.
    """
    # Initialise the array that stores the gradients of the loss with respect to the states
    grad_over_time = np.zeros((X.shape[0], X.shape[1]+1))
    grad_over_time[:,-1] = grad_out
    # Set th gradient accumulations to 0
    wx_grad = 0
    wRec_grad = 0
    for k in range(X.shape[1], 0, -1):
        # Compute the parameter gradients and accumulate the results
        wx_grad += (np.mean(grad_over_time[:,k] * X[:, k-1], axis=0))
        wRec_grad += (np.mean(grad_over_time[:, k] * S[:, k-1], axis=0))
...
# Set the small change to compute numerical gradient
eps = 1e-7
...
for p_idx, _ in enumerate(params):
    ...
    if not np.isclose(grad_num, grad_backprop):
        raise ValueError((f'Numerical gradient of {grad_num:.6f} is not close to ' f'the backpropagation gradient of{grad_backprop:.6f}!'))
