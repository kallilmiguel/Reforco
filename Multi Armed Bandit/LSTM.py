...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 25 22:10:36 2019
...
class RecurrentNeuralNetwork:
    ...
    def backProp(self):
        ...
        #forget gate
        tfu = np.zeros((self.ys, self.ys+self.xs))
        #input gate
        tiu = np.zeros((self.ys,self.ys+self.xs))
        #cell unit
        ...
        for i in range(self.rl,-1, -1):
            ...
            #gradient updates for forget, input, cell unit and output gatets + cell states & hidden states
    ...
    def sample(self):
        #loop through recurrences - start at 1 so the 0th entry of all arrays will be an array of 0s
        for i in range(1, self.rl+1):
...
class LSTM:
    ...
    #To see this, calculate the derivative of the tanh function and notice that input
    ...
    def backProp(self, e, pcs, f, i, c, o, dfcs, dfhs):
        ...
        dc = dcs * i
        #cell update = (deriv cell * activated input) * input
        ...
        iu = np.dot(np.atleast_2d(dc * self.dtangent(c)).T, np.atleast_2d(self.x))
...
for i in range(1,iterations):
    ...
    RNN.forwardProp()
    #update all our weigths using our error
    error = RNN.backProp()
    #once our error/loss is small enougth
